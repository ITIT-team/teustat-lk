...
export const GroupageTable = ({
  ...
  const [order, setOrder] = useState(
    (() => {
      ...
      return (
        JSON.parse(localStorage.getItem('groupage_tableorder')) || {
          date: 0,
          departureAndDestinationCity: 1,
          typeUnit: 2,
          rate: 3,
  ...
  useEffect(() => {
    if (
      !compareObjects(
        filter,
        INITIAL_TABS_STATE.find((t) => t.id === TAB_ID.GROUPAGE)
      )
    )
      setPagination(0)
  }, [filter])
  ...
  useEffect(
    () => sessionStorage.setItem('groupage_pagination', pagination.toString()),
    [pagination]
  )
                    ...
                    if (key === 'rate') {
                      return (
                        <th
                          ...
                          style={{ textAlign: 'left', paddingLeft: 45 }}
                          key={`th_${key}`}
                          id={key}
                        >
                          {map[key]}{' '}
