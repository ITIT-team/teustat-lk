...
export const JdTable = ({ eventAnalytic, records, filter, sorterSetter }) => {
  ...
  const [order, setOrder] = useState(
    (() => {
      ...
      return (
        JSON.parse(localStorage.getItem('jd_tableorder')) || {
          date: 0,
          departureAndDestinationCity: 1,
          containerSize: 2,
          containerOwner: 3,
  ...
  useEffect(
    () => sessionStorage.setItem('jd_pagination', pagination.toString()),
    [pagination]
  )
        ...
        {(() => {
          ...
          const map = {
            ...
            departureAndDestinationCity:
              PanelLocale['город_станция_терминал_отправления_назначения'][
                locale
              ],
