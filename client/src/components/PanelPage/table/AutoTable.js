...
export const AutoTable = ({ records, filter, sorterSetter }) => {
    ...
    useEffect(() => {
        if (!compareObjects(filter, INITIAL_TABS_STATE.find(t => t.id === TAB_ID.AUTO))) setPagination(0)
    }, [filter])

    useEffect(() => sessionStorage.setItem('auto_pagination', pagination.toString()), [pagination])
