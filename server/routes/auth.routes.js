...
rt.post(
  ...
  async (request, response) => {
    ...
    try {
      ...
      const { userId, accessLevel, accessPanel, accessAnalytics } = userData
      ...
      response
        .status(403)
        .json({ errors: [errorInHuman[e.message] || e.message] })
    }
  }
)

rt.post('/passive_authorization', verifyAccessor, async (request, response) => {
  ...
  try {
    ...
    const { userId, accessLevel, accessPanel, accessAnalytics } = newUserData
    const accessToken = jwtEncode({
      userId,
      accessLevel,
      email,
    ...
    response.cookie('teustat_token', accessToken, {
      maxAge: 2592000000,
      httpOnly: true,
    })
